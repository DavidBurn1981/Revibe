...
function goToPage(pageId){
  resetPageScroll();
  document.querySelectorAll('.modal.show,.perfOverlay.show,.successmsg.show').forEach(m=>m.classList.remove('show'));
...
document.getElementById('prev').onclick=()=>{weekStart.setDate(weekStart.getDate()-displayDays);renderCalendar()};document.getElementById('next').onclick=()=>{weekStart.setDate(weekStart.getDate()+displayDays);renderCalendar()};document.getElementById('today').onclick=()=>{weekStart=startMonday(new Date());renderCalendar()};document.getElementById('sunbedPrev').onclick=()=>{sunbedViewDate=iso(new Date(new Date(sunbedViewDate).getTime()-86400000));renderSunbedCalendar()};document.getElementById('sunbedNext').onclick=()=>{sunbedViewDate=iso(new Date(new Date(sunbedViewDate).getTime()+86400000));renderSunbedCalendar()};document.getElementById('sunbedToday').onclick=()=>{sunbedViewDate=localDateKey();renderSunbedCalendar()};document.getElementById('sunbedViewDateInput').onchange=(e)=>{if(e.target.value){sunbedViewDate=e.target.value;renderSunbedCalendar()}};function changeDiaryRange(){displayDays=+document.getElementById('diaryRange').value||7;weekStart=startMonday(new Date());renderCalendar()};
