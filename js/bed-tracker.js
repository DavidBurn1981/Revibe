...
function aggregateSessions(rows){rows=performanceSessions(rows);let total=rows.reduce((a,b)=>a+perfMinutes(b),0),paid=rows.reduce((a,b)=>a+paidMinutes(b),0);return {sessions:rows.length,minutes:total,paidMinutes:paid,signups:rows.filter(x=>x.newSignup==='Yes'||x.newSignup===true).length,rlt:rows.filter(x=>normalizeSessionType(x)==='Red Light Therapy').reduce((a,b)=>a+perfMinutes(b),0),hybrid:rows.filter(x=>normalizeSessionType(x)==='Hybrid').reduce((a,b)=>a+perfMinutes(b),0),staffMinutes:rows.reduce((a,b)=>a+(+b.staffMinutes||0),0),freeMinutes:rows.reduce((a,b)=>a+(+b.freeMinutes||0),0)}}
...
function renderPeriodPerformance(mode,refDate){
  ...
  document.getElementById('perfContent').innerHTML=
    revenueSummary+
    summaryMetricsHtml(a,kpi,paidKpi,mode==='month')+
...
function openManualAdjustmentsList(){
  let rows=(data.customerTransactions||[])
    .filter(t=>t.type==='Adjustment'&&t.product==='Manual Adjustment')
    .sort((a,b)=>(b.createdAt||'').localeCompare(a.createdAt||''));

  lastManualAdjustmentsExport=rows;
...
function exportManualAdjustmentsToExcel(){
  if(!lastManualAdjustmentsExport||!lastManualAdjustmentsExport.length){alert('No manual adjustments to export.');return}
  let sheetRows=lastManualAdjustmentsExport.map(t=>{
    let c=t.customerId?data.customers.find(x=>x.id===t.customerId):null;
    return {
      'Date':t.createdAt?formatSunbedDisplayDate(iso(new Date(t.createdAt))):'',
      'Account':c?.accountNumber||'',
  ...
  let ws=XLSX.utils.json_to_sheet(sheetRows);
  let wb=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb,ws,'Manual Adjustments');
  XLSX.writeFile(wb,`REVIBE Manually Adjusted Minutes ${localDateKey()}.xlsx`);
}
...
function searchPurchaseCustomer(){
  ...
  results.innerHTML=matches.length
    ? matches.map(c=>`<div class='customerSearchResultRow' onmousedown="event.preventDefault();selectPurchaseCustomer('${c.id}')"><b>${escapeHtml(c.firstName)} ${escapeHtml(c.lastName)}</b><div class='sub'>${escapeHtml(c.accountNumber)}</div></div>`).join('')
...
async function confirmPurchases(){
  let confirmBtn=document.querySelector('.confirmPurchaseBtn');
...
function searchSessionCustomer(){
  ...
  let matches=(data.customers||[]).filter(c=>c.active!==false&&`${c.firstName} ${c.lastName}`.toLowerCase().includes(q)).slice(0,8);
  results.innerHTML=matches.length
    ? matches.map(c=>`<div class='customerSearchResultRow' onmousedown="event.preventDefault();selectSessionCustomer('${c.id}')"><b>${escapeHtml(c.firstName)} ${escapeHtml(c.lastName)}</b><div class='sub'>${escapeHtml(c.accountNumber)}</div></div>`).join('')
    : `<div class='customerSearchResultRow muted'>No matching customers.</div>`;
  results.style.display='block';
}
