...
async function loadLiveData(){
  let [products,treatments,treatmentGroupings,renters,renterProducts,clinics,bookings,beds,sunbeds,sessions,staffMembers,staffShifts,monthlyTargets,monthlyReviewCounts,holidayRequests,dailyTakings,orders,financeOutgoings,customers,tanningProducts,customerTransactions,hours,hist,businessPlannerActions,staffRotas,apartmentCleaningTasks,customerPurchases,customerPurchaseItems,apartments,apartmentBookings]=await Promise.all([
  ...
  let err=[products,treatments,treatmentGroupings,renters,renterProducts,clinics,bookings,beds,sunbeds,sessions,staffMembers,staffShifts,monthlyTargets,monthlyReviewCounts,holidayRequests,dailyTakings,orders,financeOutgoings,customers,tanningProducts,customerTransactions,hours,hist,businessPlannerActions,staffRotas,apartmentCleaningTasks,customerPurchases,customerPurchaseItems,apartments,apartmentBookings].find(x=>x.error)?.error;
  if(err)throw err;
  ...
  data.apartmentCleaningTasks=apartmentCleaningTasks.data.map(x=>({id:x.id,date:x.task_date,apartment:x.apartment||'',apartmentId:x.apartment_id||null,note:x.note,isComplete:!!x.is_complete}));
  data.apartments=apartments.data.map(x=>({id:x.id,name:x.name,lastSyncedAt:x.last_synced_at,lastSyncError:x.last_sync_error}));
  data.apartmentBookings=apartmentBookings.data.map(x=>({id:x.id,apartmentId:x.apartment_id,checkIn:x.check_in,checkOut:x.check_out,summary:x.summary||'Reserved'}));
  ...
  data.appointments=bookings.data.map(x=>({id:x.id,clinicDayId:x.clinic_day_id,durationMinutes:+x.duration_minutes||0,date:x.booking_date,time:(x.start_time||'').slice(0,5),treatmentId:x.treatment_id,customerName:x.customer_name,customerPhone:x.customer_phone,price:+x.price,clinicDiscountPercent:+x.clinic_discount_percent,amountPayable:+x.amount_payable,notes:x.notes,clinicianNotes:x.clinician_notes,status:x.status,createdAt:x.created_at}));
  data.beds=beds.data.map(x=>({id:x.id,name:x.name,number:x.bed_number,type:x.bed_type,active:x.active}));
  data.sunbedBookings=sunbeds.data.map(x=>{let b=data.beds.find(y=>y.id===x.bed_id);return {id:x.id,name:x.customer_name,phone:x.customer_phone,date:x.booking_date,time:(x.start_time||'').slice(0,5),length:x.session_length_minutes,totalMinutes:x.session_length_minutes+x.turnaround_minutes,bed:b?.name||'',bedType:b?.type||'',sessionType:x.session_type,status:x.status,customerId:x.customer_id||null,minutesDeducted:x.minutes_deducted,bufferBeforeMinutes:+x.buffer_before_minutes||0,cancelledAt:x.cancelled_at,minutesRefunded:!!x.minutes_refunded,fulfilledBySessionId:x.fulfilled_by_session_id||null,createdAt:x.created_at}});
  data.bedSessions=sessions.data.map(x=>({id:x.id,date:x.session_date,time:(x.session_time||'').slice(0,5),length:x.session_length_minutes,payment:x.payment_type,newSignup:x.new_sign_up,purchasedBlockBooking:x.purchased_block_booking,sessionType:x.session_type,createdAt:x.created_at,cashMinutes:+x.cash_minutes||0,cardMinutes:+x.card_minutes||0,accountMinutes:+x.on_account_minutes||0,freeMinutes:+x.free_minutes||0,staffMinutes:+x.staff_minutes||0,staffMemberName:x.staff_member_name||'',rerunMinutes:+x.rerun_minutes||0,rerunReason:x.rerun_reason||'',customerId:x.customer_id||null}));data.staffMembers=staffMembers.data.map(x=>({id:x.id,name:x.name,address:x.address||'',phone:x.phone_number||'',dateOfJoining:x.date_of_joining,holidayEntitlement:+x.holiday_entitlement_this_year||0,holidaysRemaining:+x.holidays_remaining||0,colour:x.rota_colour||'#18d7e8',active:x.active}));data.staffShifts=staffShifts.data.map(x=>({id:x.id,staffId:x.staff_member_id,date:x.shift_date,start:(x.start_time||'').slice(0,5),end:(x.end_time||'').slice(0,5),hours:+x.shift_hours||0}));data.monthlyTargets=monthlyTargets.data.map(x=>({
    id:x.id,
  ...
  data.customers=customers.data.map(x=>({id:x.id,accountNumber:x.account_number,firstName:x.first_name,lastName:x.last_name,name:`${x.first_name} ${x.last_name}`,dob:x.date_of_birth,phone:x.phone_number||'',email:x.email||'',address:x.address||'',uv:x.intends_uv_or_injectables,idChecked:x.id_checked,idCheckedDate:x.id_checked_date,minutesLeft:+x.minutes_left||0,active:x.active,uvAllowed:x.uv_allowed,verifiedBy:x.verified_by||[],skinType:x.skin_type||null,generalHealthNotes:x.general_health_notes||'',bedUse:x.bed_use||'Hybrid',preferredBed:x.preferred_bed||'Any Bed',waiverSignedPresent:!!x.waiver_signed_present,bedDemoProvided:!!x.bed_demo_provided,createdAt:x.created_at,authUserId:x.auth_user_id||null}));
  data.tanningProducts=tanningProducts.data.map(x=>({id:x.id,type:x.product_type,title:x.title,description:x.description||'',minutes:x.minute_amount==null?null:+x.minute_amount,price:+x.price||0,stock:x.current_stock_level==null?null:+x.current_stock_level,active:x.active,cardMachine:x.card_machine||'Sunbed Card'}));
  data.customerTransactions=customerTransactions.data.map(x=>({id:x.id,customerId:x.customer_id,type:x.transaction_type,product:x.product_title_snapshot||'',value:+x.total_value||0,minutes:+x.minutes_change||0,balance:+x.balance_after||0,createdAt:x.created_at,notes:x.notes||''}));
