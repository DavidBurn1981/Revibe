...
async function loadLiveData(){
  ...
  data.appointments=bookings.data.map(x=>({id:x.id,clinicDayId:x.clinic_day_id,durationMinutes:+x.duration_minutes||0,date:x.booking_date,time:(x.start_time||'').slice(0,5),treatmentId:x.treatment_id,customerName:x.customer_name,customerPhone:x.customer_phone,price:+x.price,clinicDiscountPercent:+x.clinic_discount_percent,amountPayable:+x.amount_payable,notes:x.notes,clinicianNotes:x.clinician_notes,status:x.status}));
