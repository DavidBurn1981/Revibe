...
async function loadLiveData(){
  ...
  data.sunbedBookings=sunbeds.data.map(x=>{let b=data.beds.find(y=>y.id===x.bed_id);return {id:x.id,name:x.customer_name,phone:x.customer_phone,date:x.booking_date,time:(x.start_time||'').slice(0,5),length:x.session_length_minutes,totalMinutes:x.session_length_minutes+x.turnaround_minutes,bed:b?.name||'',bedType:b?.type||'',sessionType:x.session_type,status:x.status,customerId:x.customer_id||null,minutesDeducted:x.minutes_deducted,bufferBeforeMinutes:+x.buffer_before_minutes||0,cancelledAt:x.cancelled_at,minutesRefunded:!!x.minutes_refunded,fulfilledBySessionId:x.fulfilled_by_session_id||null}});
