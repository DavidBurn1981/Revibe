...
function apptHtml(a){let tr=data.treatments.find(x=>x.id===a.treatmentId),c=data.customers.find(x=>x.id===a.customerId),name=a.customerName||c?.name||'Customer';return `<div class='appt'><strong>${escapeHtml(tr?.name||'Treatment')}</strong><br>${escapeHtml(name)} · ${escapeHtml(tr?.duration||'')}m</div>`}
