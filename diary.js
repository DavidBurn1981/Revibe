function renderCalendar(){let cal=document.getElementById('calendar');cal.innerHTML='';let end=new Date(weekStart);end.setDate(end.getDate()+displayDays-1);let rangeName=displayDays===7?'Week':displayDays===14?'Next 2 Weeks':'Next 30 Days';document.getElementById('weekLabel').textContent=`${rangeName} · ${nice(weekStart)} – ${nice(end)}`;for(let i=0;i<displayDays;i++){let d=new Date(weekStart);d.setDate(d.getDate()+i);let ds=iso(d),clinic=data.clinicDays.find(x=>x.date===ds),r=clinic&&data.renters.find(x=>x.id===clinic.renterId);let el=document.createElement('div');el.className='day';el.innerHTML=`<div class='dayhead'><div class='date'>${nice(d)}</div><button class='openClinicDayBtn' onclick="openClinicDayForDate('${ds}')">${clinic?'Open Clinic Day':'Create Clinic Day'}</button>${clinic?`<div class='clinic'><strong>${clinic.product}</strong><br>${r?.name||''}<br>${clinic.start}–${clinic.end}</div>`:`<div class='clinic muted'>No clinic planned</div>`}</div>`;if(clinic){let startHour=parseInt(clinic.start.split(':')[0],10)||9,endHour=parseInt(clinic.end.split(':')[0],10)||18;let dayAppointments=(data.appointments||[]).filter(a=>a.date===ds);let slotsGrid=document.createElement('div');slotsGrid.className='slotsGrid';for(let h=startHour;h<endHour;h++){['00','15','30','45'].forEach(m=>{let t=String(h).padStart(2,'0')+':'+m;let slotStart=h*60+parseInt(m,10);let ap=dayAppointments.find(a=>a.time===t);let ongoing=!ap&&dayAppointments.find(a=>{let as=minutesFromTime(a.time),ae=as+appointmentReservedMinutes(a);return slotStart>=as&&slotStart<ae});if(ongoing)return;let slot=document.createElement('div');if(ap){let span=Math.max(1,Math.round(appointmentReservedMinutes(ap)/15));slot.className='slot';slot.style.gridRow=`span ${span}`;slot.onclick=()=>openBooking(ds,t);slot.innerHTML=`<div class='time'>${t}</div>`+apptHtml(ap)}else{slot.className='slot';slot.onclick=()=>openBooking(ds,t);slot.innerHTML=`<div class='time'>${t}</div><div class='available'>Available</div>`}slotsGrid.appendChild(slot)})}el.appendChild(slotsGrid)}cal.appendChild(el)}}
function apptHtml(a){let tr=data.treatments.find(x=>x.id===a.treatmentId),c=data.customers.find(x=>x.id===a.customerId),name=a.customerName||c?.name||'Customer';let cancelled=String(a.status||'').toLowerCase()==='cancelled by customer';return `<div class='appt${cancelled?' apptCancelled':''}'><strong>${escapeHtml(tr?.name||'Treatment')}</strong><br>${escapeHtml(name)} · ${escapeHtml(tr?.duration||'')}m${cancelled?`<br><span class='apptCancelledLabel'>CANCELLED BY CUSTOMER</span>`:''}</div>`}
...
function treatmentRoomIsFree(date,startMin,endMin){
  return !(data.appointments||[]).some(a=>{
    ...
    if(String(a.status||'').toLowerCase()==='cancelled by customer')return false;
...
function openBooking(date,time){
  let existing=(data.appointments||[]).find(a=>a.date===date&&a.time===time);
  if(existing){openBookingDetail(existing.id);return}
